...
const benefits = [
  {
    ...
    description: 'Một bảng điều khiển hiện đại giúp tổ chức theo dõi từng hoạt động trong suốt vòng đời sự kiện.'
  },
  {
    icon: Ticket,
    title: 'Vé thông minh',
    description: 'Tự động tạo vé, mã QR và quản lý check-in chỉ với một cú nhấp chuột.'
  },
  {
    icon: Users,
    title: 'Theo dõi người tham dự',
    description: 'Nắm rõ số lượng đăng ký, tình trạng ghế ngồi và phản hồi của khách tham dự.'
  },
  {
    icon: ShieldCheck,
    title: 'Phân quyền an toàn',
    description: 'Phù hợp cho Ban tổ chức, Bộ phận kiểm soát và Tình nguyện viên với các quyền hạn riêng biệt.'
  }
]
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-indigo-100 text-gray-900">
      ...
      <main className="mx-auto max-w-6xl space-y-16 px-4 py-12">
        <section className="space-y-8 text-center">
          ...
          <h1 className="text-4xl font-bold leading-tight sm:text-5xl text-gray-900">
            Trải nghiệm tổ chức sự kiện <span className="text-blue-600">thông minh</span> và{' '}
            <span className="text-blue-600">nhanh chóng</span>
          </h1>
          <p className="mx-auto max-w-2xl text-base text-gray-600 sm:text-lg">
            Dành cho Ban tổ chức, Bộ phận truyền thông và Câu lạc bộ trong hệ thống FPT Education.
            Tất cả công việc được gom gọn trong một bảng điều khiển duy nhất.
          </p>
        ...
        <section className="grid gap-4 rounded-3xl border border-gray-200 bg-white p-6 shadow-lg sm:grid-cols-3">
          {stats.map(stat => (
            <div key={stat.label} className="text-center">
              ...
              <p className="text-sm uppercase tracking-widest text-gray-600">{stat.label}</p>
            </div>
          ))}
        </section>
        ...
        <section className="space-y-6">
          <header className="space-y-2 text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.4em] text-blue-600">Tính năng</p>
            <h2 className="text-3xl font-bold text-gray-900">Tất cả trong một</h2>
            <p className="text-gray-600">Thiết kế dành riêng cho nhu cầu vận hành sự kiện nội bộ FPT.</p>
          </header>
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            {benefits.map(benefit => (
              ...
              >
                ...
                <p className="text-lg font-semibold text-gray-900">{benefit.title}</p>
        ...
        <section id="events" className="space-y-6">
          <header className="space-y-2 text-center">
            <p className="text-sm font-semibold uppercase tracking-[0.4em] text-blue-600">Sự kiện</p>
            <h2 className="text-3xl font-bold text-gray-900">Hoạt động nổi bật</h2>
        ...
        <section className="rounded-3xl border border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50 p-8 text-center shadow-lg">
          <h2 className="text-3xl font-bold text-gray-900">Sẵn sàng nâng cấp trải nghiệm sự kiện?</h2>
          <p className="mt-3 text-gray-600">
            Đăng nhập bằng tài khoản nội bộ để truy cập bảng điều khiển dành cho Event Organizer, Staff và Volunteer.
          </p>
          <Link
            to="/login"
      ...
      <footer className="border-t border-gray-200 bg-white py-6 text-center text-xs text-gray-500">
        © {new Date().getFullYear()} FPT Events Platform · Designed for SWP391 B3W Group 3
      </footer>
    </div>
  )
}
